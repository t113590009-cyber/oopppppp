...
void App::HandleLevel2ClearAnimation(float dt, float marioWorldX) {
    if (m_CastleLvl2) m_CastleLvl2->SetPosition({ 7104.0f - m_WorldOffset, -145.0f });
    if (m_FlagpoleLvl2) m_FlagpoleLvl2->SetPosition({ 6867.0f - m_WorldOffset, 6.0f });
    if (m_FlagLvl2 && !m_IsLevelClear) m_FlagLvl2->SetPosition({ 6840.0f - m_WorldOffset, 210.0f });

    if (!m_IsLevelClear && marioWorldX >= 6860.0f && marioWorldX <= 6875.0f) {
        m_IsLevelClear = true;
        m_IsFlagSliding = true;
        ...
        auto flagScoreFx = std::make_shared<ScoreEffect>(flagScore, 6867.0f - m_WorldOffset + 30.0f, hitY);
        m_Root.AddChild(flagScoreFx->GetDrawable());
        m_ScoreEffects.push_back(flagScoreFx);

        m_Player->StartFlagSlide(6867.0f - m_WorldOffset);
        m_Player->GetCharacter()->SetPosition({ 6867.0f - m_WorldOffset - 10.0f, hitY });
    }

    if (m_IsLevelClear && m_FlagLvl2 && !m_IsFlagSliding) {
        m_FlagLvl2->SetPosition({ 6840.0f - m_WorldOffset, m_FlagLvl2->GetPosition().y });
    }
    ...
    if (m_FlagLvl2 && m_IsFlagSliding) {
        ...
        if (flagPos.y > -185.0f) {
            m_FlagLvl2->SetPosition({ 6840.0f - m_WorldOffset, flagPos.y - (300.0f * dt) });
        }
        else {
            flagReachedBottom = true;
        }
        ...
        if (marioCharPos.y > groundY) {
            m_Player->GetCharacter()->SetPosition({ 6867.0f - m_WorldOffset - 10.0f, marioCharPos.y - (300.0f * dt) });
        }
        else {
            m_Player->GetCharacter()->SetPosition({ 6867.0f - m_WorldOffset - 10.0f, groundY });
            marioReachedBottom = true;
        }
    ...
    if (m_IsLevelClear && !m_IsFlagSliding) {
        if (m_WorldOffset < 6885.0f) m_WorldOffset += 150.0f * dt;

        float doorWorldX = 7104.0f;
        if (marioWorldX < doorWorldX) {
            marioWorldX += 150.0f * dt;
            m_Player->GetCharacter()->SetPosition({ marioWorldX - m_WorldOffset, m_ClearGroundY });
            m_Player->GetCharacter()->Play();
            ...
            if (m_GameTime >= 1.0f) {
                ...
                if (m_PendingFireworks > 0) {
                    ...
                    if (m_FireworkTimer <= 0.0f) {
                        ...
                        float offsetX = (m_PendingFireworks % 2 == 0) ? 60.0f : -40.0f;
                        float fwY = 50.0f + (m_PendingFireworks * 40.0f);
                        float fwWorldX = 7104.0f + offsetX;
