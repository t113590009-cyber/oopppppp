...
void App::HandleLevel2ClearAnimation(float dt, float marioWorldX) {
    // 🌟 距離校正：城堡在 7300，旗桿在 7011，旗子在 6984
    if (m_CastleLvl2) m_CastleLvl2->SetPosition({ 7300.0f - m_WorldOffset, -145.0f });
    if (m_FlagpoleLvl2) m_FlagpoleLvl2->SetPosition({ 7011.0f - m_WorldOffset, 6.0f });
    if (m_FlagLvl2 && !m_IsLevelClear) m_FlagLvl2->SetPosition({ 6984.0f - m_WorldOffset, 210.0f });

    // 碰到旗桿的瞬間 (範圍校正為 7004 ~ 7019)
    if (!m_IsLevelClear && marioWorldX >= 7004.0f && marioWorldX <= 7019.0f) {
        m_IsLevelClear = true;
        m_IsFlagSliding = true;
        ...
        auto flagScoreFx = std::make_shared<ScoreEffect>(flagScore, 7011.0f - m_WorldOffset + 30.0f, hitY);
        m_Root.AddChild(flagScoreFx->GetDrawable());
        m_ScoreEffects.push_back(flagScoreFx);

        m_Player->StartFlagSlide(7011.0f - m_WorldOffset);
        m_Player->GetCharacter()->SetPosition({ 7011.0f - m_WorldOffset - 10.0f, hitY });
    }

    if (m_IsLevelClear && m_FlagLvl2 && !m_IsFlagSliding) {
        m_FlagLvl2->SetPosition({ 6984.0f - m_WorldOffset, m_FlagLvl2->GetPosition().y });
    }
    ...
    if (m_FlagLvl2 && m_IsFlagSliding) {
        ...
        if (flagPos.y > -185.0f) {
            m_FlagLvl2->SetPosition({ 6984.0f - m_WorldOffset, flagPos.y - (300.0f * dt) });
        }
        else { flagReachedBottom = true; }

        glm::vec2 marioCharPos = m_Player->GetCharacter()->GetPosition();
        float groundY = (m_Player->IsBig() || m_Player->IsFire()) ? -216.0f : -240.0f;

        if (marioCharPos.y > groundY) {
            m_Player->GetCharacter()->SetPosition({ 7011.0f - m_WorldOffset - 10.0f, marioCharPos.y - (300.0f * dt) });
        }
        else {
            m_Player->GetCharacter()->SetPosition({ 7011.0f - m_WorldOffset - 10.0f, groundY });
            marioReachedBottom = true;
        }
