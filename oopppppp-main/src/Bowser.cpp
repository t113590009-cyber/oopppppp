...
std::shared_ptr<BowserFire> Bowser::Update(float deltaTime, float worldOffset, const std::vector<Rect>& obstacles) {
    if (m_State == State::DEAD) {
        ...
        return nullptr; // 死掉就不會噴火了
    }

    std::shared_ptr<BowserFire> spawnedFire = nullptr; // 準備用來裝新火球的變數
    ...
    if (m_FireTimer <= 0.0f) {
        // 🌟 庫巴產生火球，存進變數準備交給 App
        spawnedFire = std::make_shared<BowserFire>(m_WorldX - 30.0f, m_WorldY + 15.0f, true);
        m_FireTimer = 2.5f;
    }
    ...
    Rect nextYRect = { m_WorldX - 25.0f, nextY - 36.0f, 50.0f, 10.0f };

    bool onGround = false;
    for (const auto& obs : obstacles) {
        if (CollisionHandler::CheckCollision(nextYRect, obs)) {
            m_WorldY = obs.y + obs.height + 36.0f;
            m_VelocityY = 0.0f;
            onGround = true;
            break;
        }
